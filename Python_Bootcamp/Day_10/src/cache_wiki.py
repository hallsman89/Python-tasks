...
def get_links(title: str) -> list[dict]:
    ...
    response: requests.Response = requests.get(f"{LINK_PREFIX}/wiki/" + quote(title))
    logging.info(
...
# to storing graph in a Neo4J database:
...
def create_graph(title: str, depth: int = 3, driver: Driver | None = None) -> None:
    global page_count
    ...
    if (
        ...
        if page_count < len(mentions):
            graph[title] = [mention["title"] for mention in mentions[:page_count]]
            graph_neo4j.append(
                {
                    "title": title,
