...
def get_nodes_html(
    nodes: pd.DataFrame, graph: nx.DiGraph, node_sizes: dict
) -> alt.Chart:
...
def get_chart(graph: nx.DiGraph, positions: Mapping) -> alt.Chart:
    ...
    return (
        ...
        .encode(x="x:Q", y="y:Q", x2="xx:Q", y2="yy:Q")
        .interactive()
    )


def generate_html(graph: nx.DiGraph, positions: Mapping, node_sizes: dict) -> None:
    nodes: pd.DataFrame = (
        pd.DataFrame(positions, index=["x", "y"])
        .T.reset_index()
...
def graph_render() -> None:
    ...
    title_postitions: dict = {node: (x, y + 0.01) for node, (x, y) in positions.items()}
