from constants.constants import (
    ...
    BLUSHING_LEVEL_MAX,
    BLUSHING_LEVEL_TEXT,
    PUPILLARY_DILATION_MIN,
    PUPILLARY_DILATION_MAX,
    PUPILLARY_DILATION_TEXT,
)
...
class Condition:
    ...
    def get_avg_condition(self) -> dict[str, float]:
        ...
        return {
            ...
            "pupillary dilation": self.pupillary_dilation / self.count,
        }
    ...
    def is_valid_indicator(
        self, indicator: str, min_value: int, max_value: int
    ) -> bool:
        """Validates if the given indicator is within the specified range.
    ...
    def get_indicator(self, min_value: int, max_value: int, text: str) -> float:
        """Prompts the user for an indicator input, validates it,
        and returns the indicator as a float.
