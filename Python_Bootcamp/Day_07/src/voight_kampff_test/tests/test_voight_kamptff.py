...
class TestVoightKampff:
    ...
    @pytest.fixture
    def get_voight_kampff(self, request: pytest.FixtureRequest) -> VoightKampff:
        """Fixture that returns an instance of VoightKampff using
        a parameterized file path.
    ...
    @pytest.mark.parametrize(
        ...
        [
            (os.path.join(os.path.dirname(__file__), "test_files/1.json"), None),
            (os.path.join(os.path.dirname(__file__), "test_files/2.json"), None),
            (os.path.join(os.path.dirname(__file__), "test_files/3.json"), None),
            (os.path.join(os.path.dirname(__file__), "test_files/4.json"), None),
            (os.path.join(os.path.dirname(__file__), "test_files/5.json"), None),
            (os.path.join(os.path.dirname(__file__), "test_files/6.json"), None),
            (os.path.join(os.path.dirname(__file__), "test_files/7.json"), None),
            (os.path.join(os.path.dirname(__file__), "test_files/8.json"), None),
            (os.path.join(os.path.dirname(__file__), "test_files/9.json"), None),
            (os.path.join(os.path.dirname(__file__), "test_files/10.json"), None),
            (PATH, JSON),
        ],
        indirect=["get_voight_kampff"],
    )
    def test_get_questions(
        self, get_voight_kampff: VoightKampff, expected: dict | None
    ) -> None:
        """Tests the 'get_questions' method of VoightKampff.
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                {
                    "question": "You're watching television. Suddenly you realize there is a wasp crawling up your arm. What would you do?",
                    "options": [
                        "I'd try to capture it in a glass, and set it free outside.",
                        "I'd kill it.",
                        "Nothing, I'll pretend it's not there",
                    ],
                    "correct_answer": 0,
                }
            ),
    ...
    def test_calculate_result(
        ...
        Args:
            avg_condition (dict[str, int]):
                The average physiological condition dictionary.
            answers (list[int]):
        ...
        """
        monkeypatch.setattr(
            condition.Condition, "get_avg_condition", lambda _: avg_condition
        )
