...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="List of urls if method post and task's uuid if method get",
    )
...
async def main(method: str, data: list[str]) -> None:
    async with aiohttp.ClientSession() as session:
        if method == "post":
            async with session.post(url=URL, json=create_data(data)) as response:
                if response.status == 201:
                    print(await response.json())
        else:
            response = await session.get(f"{URL}{data[0]}")
