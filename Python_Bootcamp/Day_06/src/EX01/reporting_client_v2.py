...
class Ship(BaseModel):
    ...
    def check_model(self) -> "Ship":
        if not (
            (
                self.ship_class == "Corvette"
                and 80 <= self.length <= 250
                and 4 <= self.crew_size <= 10
                and self.armed
            )
            or (
                self.ship_class == "Frigate"
                and 300 <= self.length <= 600
                and 10 <= self.crew_size <= 15
                and self.armed
                and self.alignment == "Ally"
            )
            or (
                self.ship_class == "Cruiser"
                and 500 <= self.length <= 1000
                and 15 <= self.crew_size <= 30
                and self.armed
            )
            or (
                self.ship_class == "Destroyer"
                and 800 <= self.length <= 2000
                and 50 <= self.crew_size <= 80
                and self.armed
                and self.alignment == "Ally"
            )
            or (
                self.ship_class == "Carrier"
                and 1000 <= self.length <= 4000
                and 120 <= self.crew_size <= 250
                and not self.armed
            )
            or (
                self.ship_class == "Dreadnought"
                and 5000 <= self.length <= 20000
                and 300 <= self.crew_size <= 500
                and self.armed
            )
