...
def is_duplicate(data: dict) -> bool:
    with Session() as session:
        ships: list[Ships] = (
            session.query(Ships).filter(Ships.name == data["name"]).all()
        )
        if len(ships) == 0:
            return False
        for ship in ships:
            officers: list[Officers] = (
                session.query(Officers).filter(Officers.ship_id == ship.id).all()
            )
...
def get_officers(alignment: str) -> list:
    with Session() as session:
        return (
            session.query(
                Officers.first_name,
...
def get_traitors() -> list[dict]:
    ...
    for i in range(min(len(ally_officers), len(enemy_officers))):
        if ally_officers[i] in enemy_officers and ally_officers[i] not in traitors:
